...
url= 'https://www.dnaindia.com/search?query=alcoholics+anonymous'
options = Options()
...
try:
    ...
    for page in pages:
        ...
        if(count != 1):
            x = 'Page '+str(count) 
            try:
                page = browser.find_element_by_css_selector("[aria-label^='%s']"%x).click()
            except:
                break
        else:
            page = browser.find_element_by_class_name("gsc-cursor-page").click()
        count += 1
